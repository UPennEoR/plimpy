...
def calibration(pathToData, pathToCalibrated):
    ...
    #pathToData = str(path)
    JDs = []
    im_files = imaged_files(str(pathToData))
    for f in im_files:
        JD = os.path.split(f)[1].split('zen.')[1].split('.HH')[0]
        JDs.append(JD)
...
def make_dictionary (JD, pathToCalibrated):
    
    #load in all files for that day
    filenames = sorted(glob.glob(os.path.join(pathToCalibrated, 'zen.' + JD +'*.HH.calibrated.uvh5')))
...
def slice_day(path, JD, interval):
    master_Dict = make_dictionary (JD, str(path))
