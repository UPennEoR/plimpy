...
class uvdata_pol_calibrator():
    ...
    def data_slice(self, use_all_times=False, base_time_range=[], prod_time_range=[], use_all_frequencies=False, base_freq_range=[], prod_freq_range=[], 
        ...
        Parameters
        ----------
        base_ants, prod_ants : list
            list of antenna nums
        base_time_range, prod_time_range : list
            list of length-2, specifying the start and the end of the time range
        base_freq_range, prod_freq_range : list
    ...
    def Wirtinger_lm_cal(self, damping_para=0., update_damping_para_per_loop=False, initial_gain_amp=1.0, diagonalize=False, Niteration=50, including_autobaseline=False, verbose=False, epsilon=1e-12):
        """
        ...
        initial_gain_amp : int
            Matrix to start with take the form of [[initial_guess_scalar, 0],[0, initial_guess_scalar]]
        ...
        for iteration in range(Niteration):
            for (i,ant) in enumerate(ants):
                ...
                for (j,ant_q) in enumerate(ants):
                    if including_autobaseline or (j!=i and not including_autobaseline):
                ...
                gain_next[i] = damping_para/(1.+damping_para)*gain_prev[i] + 1./(1.+damping_para)*np.matmul(JH_D, np.linalg.inv(JH_J))
            ...
            for ant in range(len(ants)):
                for ant_r in range(len(ants)):
                    if including_autobaseline or (ant_r != ant and not including_autobaseline):
                        sum_of_residuals[iteration] += np.sum(np.abs(prod[ant,ant_r] - np.matmul(gain_next[ant][None], np.matmul(base[ant,ant_r], gain_H_next[ant_r][None])))**2, axis=0)
...
def gain_array_interpolate(gain_array, interpolate_freq_index, approach="real_imag"):
    """
    Parameters
    ----------
    gain_array : ndarray
    ...
    approach : "amp_phase" and "real_imag"
    """
        ...
        for i in range(2):
            for j in range(2):
                ...
                if approach == "real_imag":
                    interpolate_real = interpolate.interp1d(interpolate_freq_index, np.real(gain_array[ant,interpolate_freq_index,i,j]), fill_value="extrapolate")
                    interpolate_imag = interpolate.interp1d(interpolate_freq_index, np.imag(gain_array[ant,interpolate_freq_index,i,j]),fill_value="extrapolate")
                    gain_array_interp[ant,:,i,j] = interpolate_real(freqs) + 1.j * interpolate_imag(freqs)
    return gain_array_interp


def apply_gains(uvd, gain_array_, vis_units="Jy", need_interpolate=False, interpolate_freq_index=None, need_inverse=True, epsilon=1e-12):
    """
