...
class uvdata_pol_calibrator():
    ...
    def Wirtinger_lm_cal(self, damping_para=0., update_damping_para_per_loop=False, initial_gain_amp=1.0, diagonalize=False, Niteration=50, including_autobaseline=False, verbose=False, epsilon=1e-12):
        ...
        sum_of_residuals = np.zeros(Niteration)
            ...
            for ant in range(len(ants)):
                for ant_r in range(len(ants)):
                    if including_autobaseline:
                         sum_of_residuals[iteration] += np.linalg.norm(prod[ant,ant_r] - np.matmul(gain_next[ant][None], np.matmul(base[ant,ant_r], gain_H_next[ant_r][None])))
                    else:
                        if ant_r != ant:
                            sum_of_residuals[iteration] += np.linalg.norm(prod[ant,ant_r] - np.matmul(gain_next[ant][None], np.matmul(base[ant,ant_r], gain_H_next[ant_r][None])))
            
            if update_damping_para_per_loop and iteration >= 1:
                if sum_of_residuals[iteration] >= sum_of_residuals[iteration-1]:
                    damping_para *= 1.5
